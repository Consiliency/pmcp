...
- **Override allowed:** an overlay entry with the same name as a shipped/earlier entry **replaces** it (whole-entry replace, not deep-merge) — simplest, predictable.
  > **Amended 2026-08-01 (#105).** This decision still holds for `servers:` and was deliberately *not* reversed. A narrow `server_env:` overlay section was added alongside it, patching only `extra_env` on an already-existing server. Motivation: a redeclared entry is a frozen copy, so when the shipped entry later changes (package rename, install fix) the overlay silently shadows it with no signal — a gap this decision did not anticipate. Deep-merging `servers:` was considered and rejected (it would change behavior for existing overlays and leaves "how do you unset an inherited field?" unanswerable). See `plans/detailed-overlay-server-env-patch-20260801-1530.md`.
