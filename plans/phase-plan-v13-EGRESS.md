...
- [ ] IF-0-EGRESS-1 — the outbound-action gate, `src/pmcp/feedback_egress.py`:
  ...
  `FeedbackProgress` — a small thread-safe box the worker publishes into and the handler
  reads after a timeout: `publish(submission: FeedbackSubmission) -> None` and
  `latest() -> FeedbackSubmission | None`, both guarding one `FeedbackSubmission | None`
  slot with a `threading.Lock`;
  ...
  **The preview payload shape, named against `SubmitFeedbackOutput`
  ...
  | `submit_allowed` → `refused` (the server answered with an error status) | `False` | `False` | `"refused"` | yes | **no** | `browser_issue_url(...)` | `"unknown"` |
  | `submit_allowed` → `not_dispatched` (no POST left the process: the deadline refused it, or the connection failed before any bytes were sent) | `False` | `False` | `"not_dispatched"` | yes | **no** | `browser_issue_url(...)` | `"unknown"` |
  | `submit_allowed` → `dispatched_unconfirmed` (bytes were sent, no complete response arrived — including the handler's timeout path when `progress.latest()` says dispatched) | `False` | `False` | `"dispatched_unconfirmed"` | yes | **no** | `browser_search_url(...)` — **where to look, never a compose URL** | `"unknown"` |
  ...
  **The handler's timeout path is three-way, not two-way**, and reads
  `progress.latest()` to decide which: nothing published → `not_dispatched`; published
  `dispatched_unconfirmed` → that row, with the search URL; published `created` → the
  last row, reporting the issue pmcp actually saw created, with `"unknown"` visibility
  because only the probe was lost. A timeout is not evidence that nothing happened.

  `warning` is today's constant, unchanged. `authenticated` is `True` only on the last
  success row. `repository` is always the rule-1/2 result. `message` is rendered from
  ...
  **The deadline and progress contract. Read the honest claim first: a per-socket
  ...
  1. **Nothing new starts after the deadline.** `submit_feedback_issue` takes a monotonic
     `deadline` and, **immediately before dispatching the POST**, returns
     `not_dispatched` unless `time.monotonic() + _POST_PHASE_BUDGET_SECONDS <= deadline`.
     Checking merely that the deadline has not passed is not enough: a POST started one
     second before it with a ten-second socket timeout still runs nine seconds past. This
     is the rule that bounds the **act** — the one the operator was not told about.
  ...
  3. **Every fact is published the moment it is known, because the handler discards the
     ...
     That loses a fact pmcp had. The worker therefore writes `FeedbackProgress` at three
     points: `dispatched_unconfirmed` **immediately before** the opener call; then
     `not_dispatched` if that call raises a connection-setup error (nothing was sent
     after all); then `created` or `refused` the instant the response is parsed —
     **before the probe starts** — and once more with the visibility filled in if the
     probe completes. The handler reads `progress.latest()` on its timeout path.
     Publishing "dispatched" a moment before the bytes leave is deliberately pessimistic:
     telling an operator "this may exist, go look" when it does not is a cheap false
     positive, while the opposite error files a duplicate. That asymmetry is the whole
  ...
  **Thread-safety across the boundary.** `FeedbackProgress` guards a single
  `FeedbackSubmission | None` slot — one immutable snapshot, replaced whole, never
  mutated in place — with a `threading.Lock`. One writer (the worker), one reader (the
  handler), so the lock exists to publish the write, not to arbitrate contention;
  `latest()` acquires it for a single attribute read of a few microseconds, which is a
  memory barrier rather than I/O and is not the loop-blocking P-03 forbids.
  `test_the_progress_record_is_safe_to_read_while_the_worker_runs` drives a real
  concurrent read against a running worker.
...
- **Interfaces provided**: `record_pmcp_introduced_keys(keys: Iterable[str]) -> None`, `pmcp_introduced_keys() -> frozenset[str]`, `reset_pmcp_introduced_keys() -> None` (test-only seam), `managed_secret_keys_strict(project: Path | None = None) -> set[str]`; `SubmitFeedbackOutput.submission_outcome`
...
**This lane and every lane below declares its own registry isolation.**
`pmcp_introduced_keys()` is process-global, and any test anywhere that drives
`auth_connect` records into it. `tests/conftest.py`'s autouse `_reset_dotenv_provenance`
(`:151-165`) is the model, but that file is CONSENT SL-1's, so — per IF-0-PKGID-2's
precedent — each of this phase's test files declares an autouse fixture calling
`reset_pmcp_introduced_keys()` before and after every test. Nothing outside this phase
reads the registry, so containing the reset to this phase's files is sufficient.
...
- **Interfaces provided**: `PACKAGED_FEEDBACK_REPOSITORY`, `FeedbackEgressReason`, `FeedbackEgressDecision`, `evaluate_feedback_egress`, `browser_issue_url`, `browser_search_url`, `FeedbackSubmissionOutcome`, `FeedbackSubmission`, `FeedbackProgress`, `submit_feedback_issue`
- **Interfaces consumed**: `pmcp_introduced_keys`, `managed_secret_keys_strict` (SL-1); `dotenv_sourced_keys` (`src/pmcp/env_store.py`, merged); `operator_safe` (`src/pmcp/provision_gate.py`, PKGID SL-1)
- **Parallel-safe**: yes
- **Tasks**:
...
- **Interfaces consumed**: `record_pmcp_introduced_keys`, `SubmitFeedbackOutput.submission_outcome` (SL-1); `evaluate_feedback_egress`, `FeedbackEgressDecision`, `browser_issue_url`, `browser_search_url`, `submit_feedback_issue`, `FeedbackSubmission`, `FeedbackProgress`, `PACKAGED_FEEDBACK_REPOSITORY` (SL-2); `GuidanceConfig.enable_feedback_submission` (SL-3); `anyio.to_thread.run_sync`, `anyio.fail_after` (already imported, `handlers.py:21`)
- **Parallel-safe**: no
- **Tasks**:
...
| SL-4.3 | test | SL-4.2 | `tests/test_feedback_egress.py` | **the durable-provenance and post-deadline contracts; exactly these names**: `test_a_planted_token_is_still_refused_after_its_store_entry_is_removed`, `test_the_provenance_record_survives_a_second_auth_connect_in_the_other_scope`, `test_a_startup_loaded_store_token_survives_an_unrelated_auth_connect_and_is_refused`, `test_the_gate_is_asked_about_the_project_root_the_secret_writer_uses`, `test_a_submission_that_outlives_the_handler_bound_issues_no_post_afterwards`, `test_a_dispatched_but_unconfirmed_post_is_not_reported_as_submitted`, `test_a_stall_after_dispatch_reports_unconfirmed_with_a_search_url`, `test_a_creation_observed_before_the_timeout_is_reported_as_submitted`, `test_a_stall_during_the_probe_does_not_lose_the_created_issue` | `uv run pytest -q tests/test_feedback_egress.py` |
...
try:
    ...
    result = progress.latest()        # what the abandoned worker had learned
```
...
- **Single-writer region: `src/pmcp/tools/handlers.py`, owner SL-4, sole writer.** It
  is 6584 lines and the roadmap names it as the whole v13 single-writer hazard. SL-3's
  ...
  no live conflict exists; do not open any other `handlers.py` work alongside SL-3.**
...
rg -n 'record_pmcp_introduced_keys' src/pmcp/
# ^ MUST be exactly three hits: the definition, the auth_connect call beside
#   handlers.py:4972, and the startup store-load delta in cli.load_startup_env. A
#   MISSING third hit is the second panel's Q1 hole: a store key loaded at startup and
#   then dropped from the file by an unrelated auth_connect reads as operator-supplied.
...
# ^ MUST show a publish immediately before the opener call and another immediately
#   after the response is parsed, BEFORE the visibility probe runs.
