...
**What this phase adds: no source file.** SEAL writes `SECURITY.md`, one script under
`scripts/`, six test files, and the documentation set. `## Verification` asserts
structurally that `src/` is untouched, because "the phase proves and documents; it does
not add behaviour" is a property a reviewer should be able to check rather than trust.
The one exception is contracted in Execution Notes and is not a licence: if a
reproduction in SL-2 or SL-3 **does not fail closed**, the phase stops and reports —
EC-SEAL-2 and EC-SEAL-5 then demand a fix, and a criterion outranks a non-goal.
...
1. **`build_install_child_env` is the only `sanitized_subprocess_env` caller that does
   ...
   second site, and it is the reason deferral 5's residual cannot be documented as
   "no spawned server inherits it" without qualification. **Filed, not fixed** — a fix
   is a one-argument change to a merged phase's file and SEAL writes no `src/`. The
   ledger states the guarantee at the width that is true (the strip resolves the project
   root from the working directory) and states the mismatch as a separate limitation.
   SL-3 owns both assertions.
...
3. **EC-SEAL-3 is not literally satisfiable, and should not be weakened.** Read out of
   ...
   and asserted exhaustively**, so a new one cannot appear unnoticed. The criterion's
   wording is reported upward as a roadmap defect; the acceptance criterion below keeps
   its id and its text unchanged.
...
- [ ] **IF-0-SEAL-1 — the claim ledger: the falsifiability contract between `SECURITY.md`
  ...
  **The ledger** is one table, delimited by exactly one occurrence each of:
  ...
  establish; its Proof cell is exactly `—`.
  ...
  **What makes it fail. Every rule has a named failure and a fixture.**
  ...
  | R6 | `limitation_with_proof` | a `limitation` row whose Proof is not exactly `—` |
...
land. Three waves: `{SL-1, SL-2, SL-3, SL-4}` → `SL-5` → `SL-docs`.
...
- **Interfaces provided**: `scripts/check_security_claims.py` with modes `check`, `--node-ids`, `--run`; failure names R1-R16 as frozen in IF-0-SEAL-1; the frozen v13 evidence census
...
|---|---|---|---|---|---|
| SL-1.1 | test | — | `tests/test_security_claims_parser.py` | **exactly these names**, one per frozen rule plus the three modes: `test_a_wellformed_ledger_passes_every_rule`, `test_a_missing_claim_region_is_reported`, `test_a_guarantee_without_a_proof_is_reported`, `test_a_limitation_carrying_a_proof_is_reported`, `test_a_limitation_block_must_be_labelled_as_one`, `test_a_node_id_naming_a_missing_file_is_reported`, `test_a_node_id_naming_a_missing_function_is_reported`, `test_a_node_id_naming_a_method_inside_a_class_is_reported`, `test_an_uncollectable_node_id_is_reported`, `test_a_claim_marker_with_no_ledger_row_is_reported`, `test_a_ledger_row_that_no_block_cites_is_reported`, `test_a_sentence_without_a_claim_marker_is_reported`, `test_a_second_sentence_in_a_block_must_carry_its_own_marker`, `test_a_line_that_is_neither_heading_nor_claim_block_is_reported`, `test_an_abbreviation_that_breaks_the_sentence_split_is_reported`, `test_duplicate_and_non_sequential_claim_ids_are_reported`, `test_every_v13_evidence_file_must_be_cited`, `test_a_previously_unproven_test_must_be_cited`, `test_the_node_ids_subcommand_prints_the_cited_union`, `test_the_run_subcommand_fails_when_a_cited_test_fails` | `uv run pytest -q tests/test_security_claims_parser.py` |
...
**The strip pair, and why it is two tests rather than one.**
`test_an_install_spawn_strips_the_credentials_pmcp_stores_hold` asserts the behaviour
that is true today. `test_the_install_spawn_strip_resolves_the_project_root_from_the_working_directory`
asserts the *mechanism* — that `build_install_child_env` resolves the project store by
walking up from the working directory, not from the gateway's configured project root —
and it is what lets `SECURITY.md` state a guarantee at the width that is true and a
separate limitation for the `--project` mismatch. **Neither test asks for a fix**, and
SL-3 must not make one: `src/pmcp/manifest/installer.py` belongs to a merged phase, and
`## Verification` fails the phase if `src/` changes. SL-docs files the issue.
...
| Task ID | Type | Depends on | Files in scope | Tests owned | Test command |
|---|---|---|---|---|---|
| SL-5.1 | test | SL-1 | `tests/test_security_claims.py` | **exactly these names**: `test_the_shipped_security_md_passes_the_claim_checker`, `test_every_cited_node_id_exists_and_collects`, `test_every_v13_evidence_file_is_cited_by_a_guarantee`, `test_every_limitation_is_labelled_and_carries_no_proof`, `test_no_guarantee_cites_a_test_outside_the_repository_suite`, `test_the_security_policy_names_the_packaged_repository`, `test_the_subprocess_spawning_limitation_names_the_v13_gates` | `uv run pytest -q tests/test_security_claims.py` |
...
for a name in neither table, and what the strip does and does not remove; the install
spawn's project-root resolution; the abandoned worker that can occupy a thread-limiter
token without acting; and the out-of-band credential store no provenance source can see.
...
**The legacy sweep, and its two required tests.** Outside the region,
...
**Two issues to file**, each naming its reproduction: the `build_install_child_env`
project-root mismatch (Context finding 1), and `set_startup_policy` invalidating a trust
record without saying so (deferral 3). Reference Consiliency/pmcp#230 as the origin.

## Execution Notes

- **Parallelism.** Three waves: `{SL-1, SL-2, SL-3, SL-4}` → `SL-5` → `SL-docs`. The four
  root lanes share no file and need nothing from each other. SL-5 may draft `SECURITY.md`
  as soon as SL-1's format lands, because every node id it cites is frozen in this
  document — but it may not report done until all four root lanes have merged, since R8
  and R9 resolve against the tree.
- **Single-writer regions.** `SECURITY.md`, owner SL-5, sole writer. `scripts/check_security_claims.py`,
  owner SL-1. Each test file has exactly one owning lane. **No lane writes `src/`**, and
  `## Verification` asserts it: `git diff --name-only origin/main..HEAD -- src/` MUST be
  empty. That grep is this phase's non-goal made checkable.
- **The one contracted exception to "no new gates".** If any SL-2 or SL-3 reproduction
  **does not** fail closed, the lane stops and reports rather than adjusting the test
  until it passes. EC-SEAL-2 and EC-SEAL-5 require the reproductions to fail closed, and a
...
- **A fresh worktree must run `uv sync --all-extras -p 3.10` before any `uv run`**, or
  ...
  mean nothing.
- **Expected out-of-lane fallout: none, and it is measurable rather than hoped for.** No
  lane writes `src/`, so no production behaviour changes and no existing assertion can
  move. `grep -rln SECURITY.md tests/ scripts/` returns nothing — the only reference is a
...
- **Expected add/add conflicts**: none. Six new files, no lane stubs another's, and there
...
- [ ] EC-SEAL-1 — SECURITY.md describes the implemented model with no claim the tests do not prove; each claim cites the test that proves it. Proven by `uv run pytest -q tests/test_security_claims_parser.py::test_a_wellformed_ledger_passes_every_rule tests/test_security_claims_parser.py::test_a_missing_claim_region_is_reported tests/test_security_claims_parser.py::test_a_guarantee_without_a_proof_is_reported tests/test_security_claims_parser.py::test_a_limitation_carrying_a_proof_is_reported tests/test_security_claims_parser.py::test_a_limitation_block_must_be_labelled_as_one tests/test_security_claims_parser.py::test_a_node_id_naming_a_missing_file_is_reported tests/test_security_claims_parser.py::test_a_node_id_naming_a_missing_function_is_reported tests/test_security_claims_parser.py::test_a_node_id_naming_a_method_inside_a_class_is_reported tests/test_security_claims_parser.py::test_an_uncollectable_node_id_is_reported tests/test_security_claims_parser.py::test_a_claim_marker_with_no_ledger_row_is_reported tests/test_security_claims_parser.py::test_a_ledger_row_that_no_block_cites_is_reported tests/test_security_claims_parser.py::test_a_sentence_without_a_claim_marker_is_reported tests/test_security_claims_parser.py::test_a_second_sentence_in_a_block_must_carry_its_own_marker tests/test_security_claims_parser.py::test_a_line_that_is_neither_heading_nor_claim_block_is_reported tests/test_security_claims_parser.py::test_an_abbreviation_that_breaks_the_sentence_split_is_reported tests/test_security_claims_parser.py::test_duplicate_and_non_sequential_claim_ids_are_reported tests/test_security_claims_parser.py::test_every_v13_evidence_file_must_be_cited tests/test_security_claims_parser.py::test_a_previously_unproven_test_must_be_cited tests/test_security_claims_parser.py::test_the_node_ids_subcommand_prints_the_cited_union tests/test_security_claims_parser.py::test_the_run_subcommand_fails_when_a_cited_test_fails tests/test_security_claims.py::test_the_shipped_security_md_passes_the_claim_checker tests/test_security_claims.py::test_every_cited_node_id_exists_and_collects tests/test_security_claims.py::test_every_v13_evidence_file_is_cited_by_a_guarantee tests/test_security_claims.py::test_every_limitation_is_labelled_and_carries_no_proof tests/test_security_claims.py::test_no_guarantee_cites_a_test_outside_the_repository_suite tests/test_security_claims.py::test_the_security_policy_names_the_packaged_repository tests/test_security_claims.py::test_the_subprocess_spawning_limitation_names_the_v13_gates`, falsified at the structure layer by sixteen doctored `SECURITY.md` fixtures in `tmp_path`, one per frozen rule R1-R16, each asserting the checker names that rule and exits non-zero — a guarantee row with an empty Proof cell, a limitation row carrying one, a limitation block missing its `**Limitation.**` label, a marker with no ledger row and a ledger row no block cites, a claim block whose second sentence carries no marker, a loose prose paragraph inside the region, an `e.g.` that would split the sentence stream wrongly, duplicate and non-sequential ids, a census file no guarantee cites, and one of the eleven node ids no merged phase's criteria run left uncited — R17, which is what makes CONSENT amendment 1's ten unproven security properties and TRUST's one required at last, since neither merged plan's acceptance criteria can be edited now; falsified at the resolvability layer by a citation naming a file that does not exist, a function that does not exist, and a method inside a class rather than a module-level `def`, each of which resolves for a regex and must not for `ast.parse`, plus one that exists in the file but does not collect; falsified at the liveness layer by `--run` against a fixture whose cited test fails, asserting a non-zero exit — the check that a claim's proof still passes, and the reason `--run` exists at all. Falsified at the shipped-file layer by running the checker over the real `SECURITY.md`, by asserting every one of the nineteen v13 evidence files is cited by at least one guarantee, and by asserting no guarantee cites a node id outside `tests/`, without which the liveness argument fails silently — CI runs `pytest tests/`, so only citations inside the suite are executed. And at the legacy layer by asserting no `ViperJuice` occurrence survives in the file, with the advisory URL's owner compared against the packaged distribution metadata rather than a duplicated literal, and by asserting the subprocess-spawning bullet no longer reads as though nothing gates a project config entry. **Fails on `main`**: `SECURITY.md` contains no claim region, no ledger and no v13 trust model at all, `scripts/check_security_claims.py` does not exist, and `SECURITY.md:201` directs vulnerability reports to a repository this project does not own.
...
# SEAL's non-goal, made checkable: this phase adds no behaviour.
git diff --name-only origin/main..HEAD -- src/
# ^ MUST be empty. A non-empty result means a lane made a behaviour change; if a
#   reproduction genuinely failed to fail closed, that is an escalation (Execution
#   Notes), not a silent commit.
...
# ^ MUST list all nineteen merged-phase evidence files, plus the four suites this phase
#   adds that prove model behaviour. `tests/test_security_claims_parser.py` and
#   `tests/test_security_claims.py` are about the checker, not the model, and are the
#   two files a claim has no reason to cite.
...
- **What `canonical_spec_update` obliges, and why it is different from the other four
  ...
  1. **The roadmap's `## Verification` block is corrected to run the real evidence set.**
     ...
     in `## Verification` above: nineteen merged-phase files plus this phase's six.
  2. **The census is recorded, with its two new undercounts.** TRUST added three test
     ...
     `scripts/check_security_claims.py` and the six test files this phase adds.
  3. **The SEAL post-execution amendment block is written**, in the form the four merged
     phases use: what shipped, what this roadmap's SEAL text is contradicted by, and the
     items handed forward. It must record at minimum that EC-SEAL-3's literal wording is
     not satisfiable and what was proven instead; that CONSENT amendment 10a was already
     resolved in CONSENT's own merge; and the two issues filed.
...
- SL-1: effort=high, reason=this is the phase's contract rather than a helper; sixteen rules with a sentence-splitting discipline that has to be exact rather than heuristic, and a checker nobody has run is a checker nobody has tested
