...
- [ ] **IF-0-SEAL-1 — the claim ledger: the falsifiability contract between `SECURITY.md`
  ...
  **What makes it fail. Every rule has a named failure and a fixture.**
  ...
  | R9 | `not_discovered_by_ci` | a cited node id not in the set **CI's own collection discovers** — the gap between "pytest can be told to run it" and "CI will" |
  ...
  | R20 | `terminator_not_followed_by_whitespace` | after masking, a `.`/`!`/`?` inside a claim block followed by any non-whitespace character other than the block end — the ambiguity R12's split cannot resolve |
  | R21 | `unlisted_heading` | a heading inside the region that is not in the frozen label set |
  | R22 | `ci_invocation_drift` | the checker's discovery invocation no longer matches the one `.github/workflows/test.yml` runs |

  **R12's sentence rule, in full, because a heuristic here would be evadable by
  rephrasing.** Masking runs in exactly this order, **four steps since the second panel**:
  (1) inline code spans; (2) claim markers, replaced by a sentinel that is not a sentence
  terminator and is remembered; (3) markdown links, replaced by their link text;
  (4) **markdown emphasis delimiter runs — `**`, `__`, `*`, `_`, `~~` — deleted outright.**
  Markers are masked **before** links so `[C-07]` is not read as a bracket, and emphasis is
  deleted **last**, after the first three have removed every construct whose interior could
  contain a delimiter character. The remaining text of each block is split on
  `(?<=[.!?])\s+` and at the block end, and every non-empty resulting sentence must carry a
  sentinel.

  **Step 4 exists because the second plan panel ran the frozen regex and it did not fire.**
  Reproduced verbatim in this worktree, against
  `- **No install child inherits credentials.** Package identities are checked [C-01].`:
  `(?<=[.!?])\s+` does not split after `credentials.`, because the period is followed by
  `**` rather than whitespace. The splitter saw **one** sentence, the block passed on the
  second sentence's marker, and the bolded claim — the strongest assertion in the block, and
  the one an author is most likely to bold — was uncited. The italic spelling
  (`- *Nothing is submitted by default.* The gate refuses [C-02].`) failed identically. This
  is R3's shape again: a rule that exists and does not fire, in the rule EC-SEAL-1 leans on
  hardest.

  **R20 is the fail-closed half, and it is what makes step 4 a rule rather than a patch.**
  Deleting the emphasis delimiters we know about fixes the two spellings that were measured;
  it does not prove there is no third. So after masking and deletion, any `.`, `!` or `?`
  followed by a **non-whitespace character** — other than at the end of the block — is
  reported as `terminator_not_followed_by_whitespace`, rather than parsed. An HTML tag, a
  footnote reference, a bracket, an entity, a markup construct nobody has thought of: each
  is refused and the author writes the sentence plainly. This is R14's philosophy applied to
  the other side of the terminator — **forbid the ambiguity instead of enumerating the
  markup** — and it is why the sentence split can be called total rather than heuristic. R14
  handles what precedes a terminator (an abbreviation that is not one); R20 handles what
  follows it.
  ...
  **R21: a heading inside the region is navigation, never assertion.** The second panel
  found that a heading is a legal line kind carrying no marker requirement, so
  `### The install spawn strips every credential PMCP stores` would be a model assertion
  inside the region that no rule touches — and R18 does not apply *inside* the region. The
  resolution is a **frozen label set**: a heading in the region must be one of
  `### The v13 trust model`, `#### Identity, not labels`,
  `#### Consent for repository-supplied configuration`, `#### Outbound actions` and
  `#### Limitations` — the four subsections SL-5 writes, under their one parent.

  **Why a label set rather than requiring headings to carry markers.** Three reasons, and
  the second is the deciding one. It is *decidable*: "is this heading in the list" needs no
  judgement about whether a heading asserts something, whereas a marker requirement would
  leave `### Limitations` needing a citation. It **removes the incentive rather than
  taxing it** — allowing a cited heading would invite authors to put claims in headings,
  and a heading is exactly where a claim is least likely to be read carefully. And it
  matches the pattern this gate already uses twice: R16's census and R18's allowlist are
  both frozen constants, so adding a heading is a deliberate, reviewable edit to the
  checker, like adding an allowlist entry. The cost is that restructuring the section
  requires a checker change; that is the intended friction.
  ...
  **How liveness is reached without a new CI job — corrected after the second panel, which
  showed the first version's argument did not hold.** The first version said R9 ran "one
  `pytest --collect-only -q` over the union", and reasoned that because CI runs
  `pytest tests/`, every cited test therefore runs. **That inference is invalid: explicit
  collection and discovery are different operations.** Measured in this worktree — a file
  named `tests/proof_helpers.py` containing `def test_boundary()` collects when named
  explicitly (`1 test collected`) and is invisible to `pytest tests/` (`0` matches),
  because discovery is by the `test_*.py` filename pattern. Such a citation passes R7 and
  R8, passed the old R9, and would never execute again after the phase closed. The
  guarantee would have been structurally perfect and empty.

  **R9 is therefore a SUBSET check against CI's real invocation, not a collection check.**
  The cited union must be a subset of the node ids that CI's own command discovers. The
  command is read out of the repository rather than assumed: **`uv run pytest tests/ -v
  --tb=short --cov --cov-report=xml --cov-report=term-missing`**, from
  `.github/workflows/test.yml:54`; `.github/workflows/release.yml:27` runs the same
  `pytest tests/` with different reporting flags. The checker's own discovery invocation is
  `uv run pytest tests/ --collect-only -q`, which shares CI's *target* (`tests/`) and
  inherits the same `pyproject.toml` configuration, and discovery is what differs between
  the two rules — the reporting flags do not change which tests are found.

  **That configuration carries a second exclusion the subset check gets for free.**
  `[tool.pytest.ini_options]` sets `addopts = "-m 'not live'"` (`pyproject.toml:149`), and
  **19 of this suite's 1019 tests are `live`-marked and deselected**. A citation of one
  would satisfy "the function exists" and "the file is discovered" and still never run in
  CI. Measured: explicitly collecting a `live` node id reports `no tests collected (19
  deselected)`, so this half was already caught — but by accident of `addopts`, not by
  design. Under the subset rule both holes close for the same reason: **the only node ids a
  guarantee may cite are the ones CI actually discovers.**

  **R22 is the drift guard, and it is the rule that keeps R9 honest.** R9's strength is
  entirely that the checker asks the same question CI does. If someone later narrows CI's
  invocation — a path filter, a `-k`, a changed marker expression — R9 would keep passing
  against a command nobody runs. R22 parses the `pytest` invocation out of
  `.github/workflows/test.yml` and fails when it is no longer the one the checker mirrors.
  This is EGRESS amendment 5's lesson in structural form: a check is code, and a check
  written against an assumption nobody re-reads is a check that decays silently.

  **What `--run` does and does not prove, stated precisely because the first version
  conflated them.** `--run` executes the cited union once, during phase verification: it
  proves the proofs pass *at the moment the phase closes*. It says nothing about later
  changes. The property that makes CI execute them **afterwards**, on every subsequent
  commit, is R9's discovery-subset rule and nothing else. Two mechanisms, two jobs:
  `--run` is a gate at close, R9 is the standing guarantee. A claim whose test is deleted
  or renamed then fails R8 and R9; a claim whose test starts failing fails CI's own suite.
...
- **Interfaces provided**: `scripts/check_security_claims.py` with modes `check`, `--node-ids`, `--run`; failure names R1-R22 as frozen in IF-0-SEAL-1; the frozen v13 evidence census, unproven-test list, v13 lexicon, region-external allowlist and region heading label set
...
| Task ID | Type | Depends on | Files in scope | Tests owned | Test command |
|---|---|---|---|---|---|
| SL-1.1 | test | — | `tests/test_security_claims_parser.py` | **exactly these names** — one per frozen rule R1-R22, one for the accepted characterization form, the positive case, and the two modes: `test_a_wellformed_ledger_passes_every_rule`, `test_a_missing_claim_region_is_reported`, `test_a_missing_or_duplicated_ledger_is_reported`, `test_a_kind_outside_the_two_value_vocabulary_is_reported`, `test_a_guarantee_without_a_proof_is_reported`, `test_a_proof_entry_that_is_not_a_node_id_is_reported`, `test_a_limitation_carrying_an_uncharacterized_proof_is_reported`, `test_a_limitation_may_cite_a_characterization_and_nothing_else`, `test_a_guarantee_may_not_cite_a_characterization`, `test_a_limitation_block_must_be_labelled_as_one`, `test_a_node_id_naming_a_missing_file_is_reported`, `test_a_node_id_naming_a_missing_function_is_reported`, `test_a_node_id_naming_a_method_inside_a_class_is_reported`, `test_a_cited_test_that_ci_would_not_discover_is_reported`, `test_the_checkers_discovery_invocation_matches_the_ci_workflow`, `test_a_claim_marker_with_no_ledger_row_is_reported`, `test_a_ledger_row_that_no_block_cites_is_reported`, `test_a_sentence_without_a_claim_marker_is_reported`, `test_a_second_sentence_in_a_block_must_carry_its_own_marker`, `test_a_bolded_sentence_before_another_is_split_and_must_be_cited`, `test_a_terminator_followed_by_unexpected_markup_is_reported`, `test_a_line_that_is_neither_heading_nor_claim_block_is_reported`, `test_a_heading_outside_the_reviewed_label_set_is_reported`, `test_an_abbreviation_that_breaks_the_sentence_split_is_reported`, `test_duplicate_and_non_sequential_claim_ids_are_reported`, `test_every_v13_evidence_file_must_be_cited`, `test_a_previously_unproven_test_must_be_cited`, `test_a_v13_model_term_outside_the_region_is_reported`, `test_the_node_ids_subcommand_prints_the_cited_union`, `test_the_run_subcommand_fails_when_a_cited_test_fails` | `uv run pytest -q tests/test_security_claims_parser.py` |
...
| R9 | `test_a_cited_test_that_ci_would_not_discover_is_reported` |
| R10 | `test_a_ledger_row_that_no_block_cites_is_reported` |
| R11 | `test_a_claim_marker_with_no_ledger_row_is_reported` |
| R12 | `test_a_sentence_without_a_claim_marker_is_reported`, `test_a_second_sentence_in_a_block_must_carry_its_own_marker`, `test_a_bolded_sentence_before_another_is_split_and_must_be_cited` |
...
| R20 | `test_a_terminator_followed_by_unexpected_markup_is_reported` |
| R21 | `test_a_heading_outside_the_reviewed_label_set_is_reported` |
| R22 | `test_the_checkers_discovery_invocation_matches_the_ci_workflow` |
| `--node-ids` | `test_the_node_ids_subcommand_prints_the_cited_union` |
| `--run` | `test_the_run_subcommand_fails_when_a_cited_test_fails` |

Thirty tests, twenty-two rules, every rule with at least one owner and no test without a
rule (R9's test was renamed rather than added, so the second panel's five findings cost
four new tests, not five). **A lane that adds a rule must add its row here, its test to the task table, and its
...
- **Scope**: The CHANGELOG narrative (including SL-0's behaviour change), the docs catalog, the README pointer, the roadmap amendments and re-pin, and the one issue this phase files.
...
# EC-SEAL-1: the claim binding, as a reviewer runs it. `check` is structure,
# resolvability and the CI-discovery subset; `--run` additionally executes every cited
# test once and fails if one fails.
...
# Every cited node id lives under tests/. NECESSARY BUT NOT SUFFICIENT -- the second plan
# panel's B2. A tests/proof_helpers.py::test_boundary passes this and is never discovered.
uv run python scripts/check_security_claims.py --node-ids | grep -cv '^tests/'
# ^ MUST print 0.

# The rule that actually carries liveness: the cited union is a SUBSET of what CI's own
# collection discovers. This is R9, reproduced here as a reviewer runs it by hand.
comm -23 <(uv run python scripts/check_security_claims.py --node-ids | sort -u) \
         <(uv run pytest tests/ --collect-only -q 2>/dev/null | grep '::' | sort -u)
# ^ MUST be empty. A non-empty line is a claim whose proof CI will never run: a file
#   outside the test_*.py discovery pattern, or a `live`-marked test that
#   pyproject.toml's addopts = "-m 'not live'" deselects (19 of 1019 tests today).

# R22: the checker mirrors CI's invocation, so narrowing CI cannot hollow out R9.
grep -n 'pytest tests/' .github/workflows/test.yml .github/workflows/release.yml
# ^ MUST still be `uv run pytest tests/ ...` in both. test.yml:54 is the one R9 mirrors;
#   a path filter, a -k, or a changed marker expression here is the drift R22 fails on.
...
- SL-1: effort=high, reason=this is the phase's contract rather than a helper; twenty-two rules with a sentence-splitting discipline that has to be exact rather than heuristic, and a checker nobody has run is a checker nobody has tested
...
- SL-3: effort=high, reason=the seams no per-phase suite can see, including the two strip tests that assert SL-0's fixed behaviour rather than characterising the defect it removed
...
- SL-docs: effort=medium, reason=the only `canonical_spec_update` closeout in the roadmap, four spec edits, five pin updates, one issue and the CHANGELOG entry for SL-0
