...
- The cross-ecosystem instance of EC-1 (an npm cache against a docker config) is **already fixed** — a version and a digest are incomparable, so `compare_versions` returns `"incomparable"`, which is not `"not_newer"`, and the short-circuit does not fire. (2.2.1 achieved this through `are_versions_comparable`; TRISTATE deleted that function and folded the property into `compare_versions`. The behaviour is unchanged.) Only the same-ecosystem case survives. Do not re-plan the part that works; verify it first.
