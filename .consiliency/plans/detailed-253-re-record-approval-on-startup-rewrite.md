...
- `set_startup_policy` — modify — bind approval to a single captured input snapshot, in this exact order:
  ...
  - **Pin the canonical trust key; do not let recording re-resolve a mutable path (panel finding, codex — path-identity race).** Both `is_approved` and `record` currently call `Path(path).resolve()` internally, so a symlink swap of `target.path` BETWEEN the approval check and the record (or between the write and the record) redirects resolution: `A` is approved, then `A` is replaced by a symlink to a previously-unapproved `B` that happens to hold `output_bytes`, and `record(target.path, ...)` resolves `A→B` and approves `B` — transferring consent to a file the operator never approved, violating the no-new-approval rule. Closing only the byte-content race is not enough. The fix binds consent to ONE canonical identity captured up front: resolve the trust key exactly once (alongside `input_bytes`), and use that SAME pinned key for both the approval lookup and the record WITHOUT resolving `target.path` again during recording. Because `record`/`is_approved` re-resolve today, this requires a small, separately-reviewed change to `trust_store`: either (a) a keyed variant `record_resolved(resolved_path, content, ...)` / `is_approved_resolved(...)` that treats the given path as the canonical key verbatim, or (b) capturing the identity from an OPEN file descriptor (open once with `O_NOFOLLOW` on the final component, operate via that fd's canonical path) so a symlink substituted at the path cannot redirect the recorded key. Prefer (b) if the write path can carry the fd; (a) is acceptable if the resolve-once key is demonstrably immune to a later swap. Decide during implementation; the invariant is: **the path approved is the exact path whose bytes were gated, pinned once, never re-resolved after.**
