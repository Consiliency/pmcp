...
# GENERATED by .consiliency/notes/derive_npm_flags.py -- do not hand-edit.
#
# The boolean spellings whose declared type is `null|Boolean` -- these, and
# ONLY these, also consume a literal `null`. Verified against nopt: `npm exec
# --yes null zz` leaves `zz`, but `npm exec --global null zz` leaves `null zz`,
# so `null` is the package for every NON-nullable boolean. A blanket `null`
# rule therefore minted a wrong identity across the other ~180 boolean entries
# -- and `null` is a real published npm package (ah board review, correctness
# seat).
#
# Hand-listing it was the other way to get this wrong, and 2.5.1 did: the five
# nullable definitions have EIGHTEEN spellings, because `y`, `ws`, `n` and `no`
# are shorthands (`n` and `no` both expand to `--no-yes`) and each is emitted
# in both its `-x` and `--x` form. The hand-written table carried twelve, so
# `--y -ws -n --n -no --no` each minted the package `null`. The set is now
# derived -- a spelling is nullable iff its resolution target, after shorthand
# expansion and after stripping a leading `no-`, is a definition whose declared
# type includes `null` -- and `derive_npm_flags.py --verify` re-probes every
# spelling against nopt, so a future npm cannot change it silently.
