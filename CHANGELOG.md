...
### Fixed
- **Tests no longer assert upper bounds on wall-clock time.** The parallel-connection test proves concurrency with a rendezvous instead of a 0.2 s margin, and the suite's nine other `elapsed < X` assertions are replaced by the deterministic property each stood in for. New `tests/_timing.py` (`eventually`, `eventually_sync`, `Rendezvous`) is the shared wait primitive. See [#235](https://github.com/Consiliency/pmcp/issues/235), see [#226](https://github.com/Consiliency/pmcp/issues/226).
