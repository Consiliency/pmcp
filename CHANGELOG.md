...
### Fixed
- **Exact-version validation follows npm's classification of tarball specs.** `is_valid_package_version` now refuses a version ending in `.tgz`, `.tar` or `.tar.gz` (any case), matching npm-package-arg's `isFileType` rule, which npm applies before reading a selector as a registry version. Every path that composes a version into `npx` argv (provision gate, package approvals, installer, CLI) inherits it.
