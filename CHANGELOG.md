...
### Changed
- **`pmcp refresh --check-versions` now reports a local server whose package it
  cannot classify as stale on every run.** A server launched as
  `node /opt/srv.js` or `python -m thing` has no classifiable package, so its
  configured identity is unknown, and "cannot confirm identity" resolves to
  refresh. Such a server now prints `local: 1.0.0 -> unknown` and the
  `Run 'pmcp refresh --force' to update.` footer **every time the check runs,
  permanently** — running the refresh does not settle it, because the next check
  still cannot classify the package. Previously these servers were skipped in
  silence. This is a known rough edge, stated plainly rather than buried: it
  follows unavoidably from failing safe, since the only alternative is to read
  "cannot classify" as "assume it matches", which is the fail-open reading that
  let a swapped package look current in the first place. A manifest of only
  registry-installed (npm/pypi/cargo/docker) servers is unaffected.
