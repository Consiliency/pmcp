...
### Changed
- **`pmcp startup add/set --source project` now carries your prior trust approval forward when it rewrites `.mcp.json`.** Setting the startup policy changes the file's bytes, and trust approval is content-keyed, so the edit used to silently invalidate your own `pmcp trust approve` of that file and the next startup refused it. When the pre-write bytes were approved, pmcp now re-records the approval for the exact bytes it writes — keyed on one file identity pinned atomically with the read, never re-approving a file that was not already approved, and never approving a substituted file. A symlinked `.mcp.json` is refused up front, and a checkout-resident trust store leaves the file written with a diagnostic rather than crashing. See [#253](https://github.com/Consiliency/pmcp/issues/253).
